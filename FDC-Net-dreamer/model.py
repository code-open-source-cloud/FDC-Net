...
class EEGSpecificTransformer(nn.Module):
    ...
    def forward(self, x, feedback=None):
        ...
        if feedback is not None:
            feedback = self.feedback_proj(feedback).unsqueeze(1)
            x = x * (1 + feedback)
...
class EnhancedDualPathEncoder(nn.Module):
...
class JointDenoiseClassify(nn.Module):
    ...
    def __init__(self, in_channels=14, length=128, num_classes=2):
        super().__init__()
        self.encoder = EnhancedDualPathEncoder(in_channels, length)
    ...
    def forward(self, x):
        ...
        if self.feedback_memory is not None:
            feedback = self.feedback_proj(self.feedback_memory.detach())
        ...
        self.feedback_memory = torch.sigmoid(logits.detach())
